...
S_set = jnp.arange(S)
A_set = jnp.arange(A)
threshes = jnp.array([0.5 / (1 - DISCOUNT)])
...
def create_P(mu1: float, mu2: float, rho: float):
    ...
    # when s==N
    P = P.at[N, 0, N].set(rho * mu1 + (1-rho) * (1-mu1) + (1 - rho) * mu1)
    P = P.at[N, 0, N-1].set(rho * (1 - mu1))
    P = P.at[N, 1, N].set(rho * mu2 + (1-rho) * (1-mu2) + (1 - rho) * mu2)
    P = P.at[N, 1, N-1].set(rho * (1 - mu2))
...
def create_rcmdp(int):
    ...
    U = np.zeros((URES ** 3, S, A, S))
    i = 0
    for mu1 in np.linspace(MU1 - eps / 2, MU1 + eps / 2, URES):
        for mu2 in np.linspace(MU2 - eps / 2, MU2 + eps / 2, URES):
            for rho in np.linspace(RHO - eps / 2, RHO + eps / 2, URES):
